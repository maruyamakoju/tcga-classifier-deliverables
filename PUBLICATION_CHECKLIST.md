...
Status for `v2.4.0-gdc-starcounts`: released and public. Every item below has been
...
- [x] Replace every explicit TBD field in `GITHUB_RELEASE_v2.4.0.md` from the
      final generated sidecar.
- [x] Upload `tcga-tumor-normal-release-lite.zip` as the GitHub Release asset.
- [x] Paste `GITHUB_RELEASE_v2.4.0.md` as the GitHub Release body.
...
- [x] State external-metric provenance: committed metrics are a historical
      snapshot; a live re-validation reproduced all three cohorts' committed
      metrics against current GDC/Xena sources (external-validation/REVALIDATION.md).
